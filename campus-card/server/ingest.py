...
def load_banks(session):
    # If you add a banks.csv, it should at least have columns: id, bank_name
    path = csv_path("banks.csv")
    if not os.path.exists(path):
        # No banks.csv: skip. You can insert banks manually or add the CSV.
        return

    df = load_csv("banks.csv")
    for _, row in df.iterrows():
        obj = models.Bank(
            id=int(row["id"]),
...
def load_events(session):
    ...
    for _, row in df.iterrows():
        ...
        obj = models.Event(
            ...
            is_volunteering=bool(row["is_volunteering"]),
...
def load_users(session):
    ...
    for _, row in df.iterrows():
        friend_id = None
        if "friend_id" in row and not pd.isna(row["friend_id"]):
            friend_id = int(row["friend_id"])

        obj = models.User(
            # id is auto-generated by DB; users.csv has no id column
            name=row["name"],
            email=row["email"],
            address=row["address"],
